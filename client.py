...
from data.config import TAIKO_ABI
...
#             from_ = self.address
        # contract = self.w3.eth.contract(
        #         address=Web3.to_checksum_address(to),
        #         abi=self.default_abi
        #     )
        contract = self.w3.eth.contract(
            address=Web3.to_checksum_address(to),
            abi=Client.taiko_abi
        )
        ...
        # try:
            # print('7', tx_params)
            # print('9', int(self.w3.eth.estimate_gas(tx_params)))
            # print('8', self.w3.eth.estimate_gas({'to': tx_params['to'], 'from': tx_params['from']}))
            # tx_params['gas'] = int(self.w3.eth.estimate_gas({'to': tx_params['to'], 'from': tx_params['from']}) * increase_gas)



        # except Exception as err:
        #     print(f'{self.address} | Transaction failed 8 | {err}')
        #     return None

        # print('tx_params', tx_params)

        abi_data = {
            'destChainId': self.w3.eth.chain_id,
            'to': self.address,
            'token': Web3.to_checksum_address('0x0469760d321D08AB4fce75E2E799902C9f55dA59'),
        ...
        # tx_params['op'] = abi_data
        tx_params['data'] = contract.encodeABI('sendToken', {'op': abi_data})
...
#
        sign = self.w3.eth.account.sign_transaction(tx_params, self.private_key)
        # res = self.verif_tx(tx_hash=sign.hash)
        # print('sign', res)
